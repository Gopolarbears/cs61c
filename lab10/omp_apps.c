...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i=omp_get_thread_num(); i<ARRAY_SIZE; i+=num_threads)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    int chunk_num = ARRAY_SIZE / num_threads;

    for(int i=thread_num*chunk_num; i<(thread_num == num_threads - 1 ? ARRAY_SIZE : (thread_num + 1)*chunk_num); i++)
      z[i] = x[i] + y[i];
  }
}
// END PART 1 EX 2
